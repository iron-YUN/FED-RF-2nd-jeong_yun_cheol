...
form.logF input[type=password]`;
  ...
  $(tgInput).blur(function () {
    ...
    ******************************************/
    ...
    // trim() 은 중간 공백제거는 안하므로
    // 모든공백을 제거하는 함수를 만들어쓴다.
    const groSpace = (x) => x.replace(/\s/g, "");
    // get rid of space 즉 공백제거함수
    // 정규식은 슬래쉬(/) 사이에 쓰며
    // |s는 스페이스기호 이고 g는 전역(global) 플래그다
    // 플래그는(flag)는 기술용어로 처리표시기호를 뜻함
    ...
    ****************************************/
      ...
      if (!vReg(cv, cid)) {
        ...
        $(this).siblings(".msg").text("멋진 아이디 입니다!").addClass("on");
        // 클래스 on 녹색글자
      } /////// else //////
    } ///////// else if ////////
    /**************************************** 
  ...
  $("#email1, #email2").on("keyup", function () {
    ...
    let backEml =
      cid == "email2"
        ...
        ? seleml.val()
        : eml2.val();
    // 현재입력 아이디가 "email1"이면 직접입력창을 읽고
    // 아니면  선택박스값이"free"가 아닌경우 선택박스값읽고
