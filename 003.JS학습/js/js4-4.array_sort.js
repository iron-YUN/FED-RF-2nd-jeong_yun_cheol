...
function sortingFn(evt, cta, arrData, exBox) {
  ...
  // 전달된 배열값을 변수에 그냥할당하면 얕은복사가 되어서
  // 정렬시 원본의 정렬을 막을수없다.
  // 따라서 깊은복사를 하여 원본원본과 분리시킨다
  // 일반적인 값의 배열 깊은복사
  // 새변수 = [원본배열변수] - 이렇게하지만
  // 효과있는 방밥?
  // 새변수 = JSON.parse(JSON.stringify(배열원본변수))
  const xxx = JSON.parse(JSON.stringify(arrData));
  // xxx는 원본과를 분리된 같은 배열값의 새로운 배열이다!
...
// console.log(searchCta4,btnSearch,keyWord,btnTotal);
...
mFn.addEvt(sel4, "change",
 (e) => sortingFn(e, cta4.value, 
  ...
  //searchResult 값에 할당되어 true이면 이값을 보내고
  // 아니면 list2를 보낸다!
...
mFn.addEvt(sel5, "change",
 (e) => sortingFn(e, cta5.value, 
  searchResult?searchResult:arrList3, showList5));
  // list3 는 객체이므로 값배열로 변환한 배열을 보낸다!
