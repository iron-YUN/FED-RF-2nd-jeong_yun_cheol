...
// oort()는 기본 문자로 처리하므로 숫자는 내부함수로 빼기 연산처리함!
// // // console.log('숫자배열:',arrNumber);
// // // console.log('숫자배열 오름차순sort()',arrNumber.sort());
// // // console.log('숫자배열 오름차순sort((a,b)=>a-b)',arrNumber.sort((a,b)=>a-b));

//예)
// sort() 만쓰면 맨앞자리가 같으면 그다음자리의 크기를 비교함
// 오름차순sort((a,b)=>a-b) 에서 붙어있는 두개의값의 차를 이용해 순서를 판단함
const arrNumber2 = [3112, 1120, 123, 284, 489, 351, 518, 513, 805, 2248];
// // // console.log('숫자배열 오름차순sort()',arrNumber2); // 있는그대로 나열
// // // console.log('숫자배열 오름차순sort()',arrNumber2.sort()); //숫자를 문자형으로 비교나열
// // // console.log('숫자배열 오름차순sort((a,b)=>a-b)',arrNumber2.sort((a,b)=>a-b)); //숫자끼리 빼서 비교함
// // // console.log('숫자배열 오름차순sort((a,b)=>a-b)',arrNumber2.sort((a,b)=>b-a)); //숫자끼리 빼서 비교함 + 역순됨
...
const showImgNum = (arrObj) => { // arrObj 전달될 배열
...
2.원본배열은 그대로 보존됨!
3.리턴키워드나 변수,함수 등 처리방법 불필요
4.이를 변수에 할당하면 새로운 배열이 생성됨!!!
5. 문자열로 찍어주려면 변수메서드 join()사용!
-> join('') 빈문자열 결합을 사용하면 배열값이 깨끗하게 문자열 덩어리로 그대로출력된다!!
-> 배열.map().join('') -> 배열 맵쪼잉~!
-> join()을 안쓰면 배열의 기본값이 콤마로 열결되어 할당되는데
콤마를 없애려면 반드시 join()을 써야함!!!! 
***************************************************************/
// // // console.log('원본배열:',arrNumber2); 
// // // console.log('원본배열로태그작성:',arrNumber2.map(v=>`<숫자>${v}</숫자>`)); 
// // // console.log('원본배열로태그작성한 배열을 문자열로 변경하기:',arrNumber2.map(v=>`<숫자>${v}</숫자>`).join('')); 
// // // console.log('원본배열로데이터작성:',arrNumber2.map((v,i)=>`🐨회원번호${i+1}:${v}포인트`)); 
...
// mFn.addEvt(selBox,'change',function(e){changeSort(e,arrNumber)});
// mFn.addEvt(selBox,'change',(e)=>{changeSort(e,arrNumber)});
mFn.addEvt(selBox,'change',(e)=>changeSort(e,arrNumber));
// = selBox.addEvt('change',changeSort);
// (5-3) 정렬변경함수 만들기
function changeSort(e,arrObj) { //arrObj 배열 전달변수
    // e -> 이벤트발생 요소의 전달되 이벤트변수
    // arrObj 배열전달변수 -> 원본배열을 담음(주소복사용!)
    // 원본배열을 보존키위해 깊은복사를 함!
    // 배열값이 일반배열값이므로 스프레드 연산자 사용!
    arrObj = [...arrObj]; // 오른쪽 arrObj 를 스프레드하여 다시 arrObj 에 담음
    // -> 다시 새로운 배열로 값이 복사됨!!
...
function sortingFn (evt,cta,arrData,exBox){
  // evt - 이벤트 발생요소의 이벤트 객체 전달
  // cta - 정렬기준값(객체속성명 :key값)
  // arrData - 배열데이터
  // exBox - 출력대상박스
...
function searchingFn(){
  ...
  let result = list2.filter(v=>{
    // v는 배열값
    if(String(v[cta]).indexOf(kword)!=-1){
      return true;
      // 숫자이면 에러남 indexOf 은 문자혈 전문
