...
function changeSort(e,arrObj) { //arrObj 배열 전달변수
    ...
    //  let optVal = event.currentTarget.value;
    ...
    let selId = e.currentTarget.id;
     ...
     if (optVal==1){
        ...
        // ->해석 : 앞뒤 같으면 0 , 뒤가 크면 -1 , 앞이크면 1
        // 0 아무것도안함 ,1 순서바꿔서 유지 , -1 그대로 유지!

        // 문자든 숫자든 sort() 메서드의 내부적 처리에서
        //앞뒤문자가 같으면 0, 뒷문자가 크면 -1
        ...
        // ->[내부적 처리란?]
        // 문자일 경우 '가'>'나' 1로처리할 경우
        // '나','가' 로 순서를 바꿔서 처리 (내림차순)
        // 즉,문자열도 순서대로 글자 알파벳,가나다라 순
        // 등 특정순서 기준이 브라우저에 구현되어있음
    ...
    //  (주의!!!!) 원본배열을 정렬후엔 원본배열은 없어진다!!!!! ㅠㅠㅠㅠㅠㅠㅠㅠ
    //  원본배열을 다른변수에 할당후 다른변수를 정렬후엔
    // 여전히 원본배열은 없어진다!! ㅠㅜㅜㅠㅜㅠㅜㅠ
...
const showSpanText = (arrObj) => { // arrObj 전달된배열
...
// (4-2) 이벤트연결하기 : 이벤트종류 : change
// 연결된 함수는 위의 숫자정렬한 정렯함수를 사용한다!
