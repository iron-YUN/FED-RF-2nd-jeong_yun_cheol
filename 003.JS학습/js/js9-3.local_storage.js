...
/*************************************************************** 
    ...
    쿠키와같이 사용자 동의를 필요로하는 로컬 기록수단의 단점은
    이것을 거부하면 쓸수없는데 , JS스토리지는 이런제약이없다
    보안상 안정이 보장되므로 사용가능
    (2) 단점: 데이터의 지속 보장이 없다!
...
function makeObj() {
  ...
  // 만약 배열데이터를 직접 넣으려고하면
  // 로컬쓰는 문자형만 받기때문에 데이터형 이름만 문자형으로 데이터를 대신 넣게된다! 즉, 배열데이터는 못들어간다 - 배열데이터는 문자형으로 변환해 넣어야 들어간다
...
function bindData() {
  ...
  console.log("게시판 화면에 뿌려", localData);
  ...
  mFn.qsa(".del-link a").forEach((ele) => {
    ele.onclick = (e) => {
      // 1.기본이동막기 = a요소 상단이동을 막음
      e.preventDefault();

      // 2.지울지 여부확인 (confirm대화창 사용)
      // confirm(메시지) -> 확인(true)/취소(false)
      if (!confirm("리얼리얼리얼루 지우시게?")) return;
      // confirm() 앞에 not연산자를 써서 falsel일때 리턴
...
// CRUD 크루드!!!
// Create 만들기 / Read 읽기 / Updata 수정 / Delete 삭제
...
mFn.addEvt(selBox, "change", (e) => {
  ...
  let selRec = localData.find((v) => {
    ...
    // 선택idx와 순회하는 배열idx 일치시
    // 이것을 저장하는 시그널 return true다!
  }); ///////////find///////////////
...
mFn.qs("#mobtn").onclick = () => {
  ...
  // 5.배열데이터에서 읽어온 옵션값 idx와 비교하여 데이터 업데이트하기
  // 배열.find(v=>if(조건){병경코드;return true})
  localData.find((v) => {
    if (v.idx == optVal) {
      // 해당항목값 업데이트하기
...
// [2. 세션 스토리지 연습]
// 1. 버튼기능 이벤트 대상 : .session-box button
