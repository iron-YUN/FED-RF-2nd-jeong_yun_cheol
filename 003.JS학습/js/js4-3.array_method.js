...
// 3-1 처음배열 출력 ///////////
// 밑에서 fruit배열 변경시 다시 출력해야하므로 함수로 변경
...
// 객체 안에서 { 키1:값1,키2:값2,키3:값3.......,}
// Object.keys 는 키 , Object.values 는 값
// 변환목적 : 배열메서드를 사용하기 위함
...
// 화살표 함수에서 중괄호를 사용하면 return하 return키워드를써줘야한다
// 따라서 중괄호 생략시 return 키워드 자동생략
...
function showFruit() {
  ...
  // (6) '중간배열삭제' 버튼 : splice()
  // 삭제시 : splice(순번) -> 해당순번부터 뒤를 모두삭제
  // (순번,개수)-> 해당순번부터 개수만큼삭제
  else if (btxt === "중간배열삭제") {
    // 지울 순번 읽어오기 (대상: #anum -> aNum)
    let delSeq = aNum.value;
    // 지울 개수 읽어오기 (대상: #delnum -> delNum)
    let delCnt = delNum.value;

    console.log("삭제대상", delSeq, "\n지울개수", delCnt);
