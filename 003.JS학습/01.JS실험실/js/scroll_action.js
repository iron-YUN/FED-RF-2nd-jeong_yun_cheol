...
function showIt() {
  ...
  // -> 아래와 작동이 동일하고 for of문이 고려하는것이 적기에 조금더 빠르다
  ...
  // if(pos<0) scAct[1].classList.add('on');
...
////////////////////// [떨녀 구현하기] /////////////////////////////
// 기본원리 : 스크롤 이동에따른 화면 높이값 범위안에서
// 떨녀 이미지가 아래쪽으로 이동애니함
// 비례식을 세운다!!
// 비례식 ->>> 스크롤한계값 : 윈도우높이
// ->>> 스크롤이동값 : 이미지이동값
// 이미지이동값 =  윈도우높이 * 스크롤이동값 / 스크롤한게값
// 이미지이동값 =  winH * scTop / scLimit

// 0.변수값 셋팅하기
//(1)스크롤 한계값 : 전체document높이 - ghkausshvdl
// 전체document높이
...
function moveWoman() {
  ...
  // 이미지이동값 =  윈도우높이(winH) * 스크롤이동값(scTop) / 스크롤한게값(scLimit)
