...
->current를 써야하는 이유는 useRef컴포넌트 객체에서 이변수를 관리해주고
값을 유지 없데이트 하는데 사용하는 속성으로
current 를 사용하도록 지정함
...
********************************************************/
// [4]컴포넌트 외부 일반변수
// (1) 전역변수 역할을 하지만 컴포넌트가 여러개 사용될때
// 이변수를 업데이트하면 여러가지 프로그래밍이 꼬인다
// 그러므로 절대절대절대 변경되는변수를 바깥쪽에 쓰지않는다!
// (2) 단, 변경되지않는 데이터 const 변수나 공유 함수 등은
// 컴포넌트 바깥코딩할수있다!
let what = "난뭐임?";
