...
function EventShow() {
  ...
  const showAladin = () => {
    ...
    ReactDOM.render(<MakeImg isrc="./images/ala4.jpg" ialt="알라" />, alaBox);
    // 컴포넌트 호출시 전달변수를 셋팅하여 보내야하는데
    // 만약 전달변수이름이 잘못되었거나 보내주지 않으면
    // 컴포넌트에서 에러가 나지 않고 해당 항복을
    // 제외하여 표시하지 않는 특징이 잇음!
    ...
    setTimeout(() => {
      ...
      // mFn.qsa("button")[0].style.margin = "0 auto";
    }, 3000);
  }; /////////////// showAladin 함수 ////////////
  ...
  const getLamp = () => {
    ...
    // 2. 램프이미지 넣기
    // ReactDOM.render(넣을코드,넣을장소) : getLamp 실행시 넣기
    ReactDOM.render(
      <MakeImg
  ...
  return (
    <React.Fragment>
      <div id="tbox" style={{ textAlign: "center" }}>
        ...
        <MakeImg
          isrc="./images/gini.avif"
          ialt="지니"
          /* 마우스오버스 showAladin 함수 */
          overFn={showAladin}
        />
        ...
        <button onClick={getFerrari}>소원빌기~! 페라리줘~!!</button>
...
function MakeImg({ isrc, ialt ,icss,overFn,itit,idName,clickFn}) {
  // 리턴코드 : return 키워드 바로뒤에 JSX태그를 바로 이어쓰거나
